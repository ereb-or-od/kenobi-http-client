...
func Unmarshalc(c *HttpClient, ct string, b []byte, d interface{}) (err error) {
	if IsJSONType(ct) {
		err = c.Marshaller.Unmarshall(b, d)
...
func jsonMarshal(c *HttpClient, r *HttpRequest, d interface{}) ([]byte, error) {
	if !r.jsonEscapeHTML {
		return noescapeJSONMarshal(d)
	} else if !c.jsonEscapeHTML {
		return noescapeJSONMarshal(d)
	}
	return c.Marshaller.Marshall(d)
}
...
func composeHeaders(c *HttpClient, r *HttpRequest, hdrs http.Header) string {
