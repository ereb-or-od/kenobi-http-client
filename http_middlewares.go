...
const debugRequestLogKey = "__HttpClientDebugRequestLog"

func parseRequestURL(c *HttpClient, r *HttpRequest) error {
	...
	// If HttpRequest.URL is relative path then added c.BaseUrl into
	// the request URL otherwise HttpRequest.URL will be used as-is
	if !reqURL.IsAbs() {
		r.URL = reqURL.String()
...
func parseRequestHeader(c *HttpClient, r *HttpRequest) error {
...
func parseRequestBody(c *HttpClient, r *HttpRequest) (err error) {
...
func createHTTPRequest(c *HttpClient, r *HttpRequest) (err error) {
	if r.bodyBuf == nil {
...
func addCredentials(c *HttpClient, r *HttpRequest) error {
	...
	if !c.DisableWarn {
		if isBasicAuth && !strings.HasPrefix(r.URL, "https") {
			c.log.Warn("Using Basic Auth in HTTP mode is not secure, use HTTPS")
		}
	}

	// Set the Authorization ExtractHeader Scheme
...
func requestLogger(c *HttpClient, r *HttpRequest) error {
	if c.Debug {
		rr := r.RawRequest
...
func responseLogger(c *HttpClient, res *HttpResponse) error {
	if c.Debug {
		rl := &ResponseLog{Header: copyHeaders(res.ExtractHeader()), Body: res.fmtBodyString(c.debugBodySizeLimit)}
		...
		c.log.Debug(fmt.Sprintf("%s", debugLog))
	}

	return nil
}

func parseResponseBody(c *HttpClient, res *HttpResponse) (err error) {
	if res.ExtractStatusCode() == http.StatusNoContent {
		return
	}
	// Handles only JSON or XML content type
	ct := firstNonEmpty(res.Request.forceContentType, res.ExtractHeader().Get(hdrContentTypeKey), res.Request.fallbackContentType)
	if IsJSONType(ct) || IsXMLType(ct) {
		// HTTP status code > 199 and < 300, considered as ExtractResult
		if res.IsSuccess() {
		...
		// HTTP status code > 399, considered as ExtractError
		if res.IsError() {
...
func handleMultipart(c *HttpClient, r *HttpRequest) (err error) {
...
func handleFormData(c *HttpClient, r *HttpRequest) {
	formData := url.Values{}
...
func handleContentType(c *HttpClient, r *HttpRequest) {
...
func handleRequestBody(c *HttpClient, r *HttpRequest) (err error) {
	var bodyBytes []byte
	contentType := r.Header.Get(hdrContentTypeKey)
	kind := kindOf(r.Body)
	r.bodyBuf = nil
	...
	if bodyBytes == nil && r.bodyBuf == nil {
		err = errors.New("unsupported 'ExtractBody' type/value")
	}
...
func saveResponseIntoFile(c *HttpClient, res *HttpResponse) error {
	if res.Request.isSaveResponse {
		file := ""
...
func getBodyCopy(r *HttpRequest) (*bytes.Buffer, error) {
