...
type (
	...
	RequestMiddleware func(*HttpClient, *HttpRequest) error

	// ResponseMiddleware type is for response middleware, called after a response has been received
	ResponseMiddleware func(*HttpClient, *HttpResponse) error
	...
	ErrorHook func(*HttpRequest, error)
)
...
func (c *HttpClient) new() *HttpRequest {
	return c.NewRequest()
}
...
func (c *HttpClient) UseRedirectPolicy(policies ...interface{}) *HttpClient {
	for _, p := range policies {
		if _, ok := p.(HttpRedirectionPolicy); !ok {
			c.log.Warn(fmt.Sprintf("%v does not implemented.HttpRedirectionPolicy (missing Apply method)",
				functionName(p)))
		}
	}
	...
	c.httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		for _, p := range policies {
			if err := p.(HttpRedirectionPolicy).Apply(req, via); err != nil {
				return err
			}
		}
...
func (c *HttpClient) AddRetryAfterErrorCondition() *HttpClient {
	c.AddRetryCondition(func(response *HttpResponse, err error) bool {
...
func (c *HttpClient) onErrorHooks(req *HttpRequest, resp *HttpResponse, err error) {
	if err != nil {
...
func createClient(logger logging.Logger, marshaller interfaces.Marshaller, hc *http.Client) (*HttpClient, error) {
	if logger == nil{
		return nil, errLoggerMustBeSpecified
	}

	if marshaller == nil{
		return nil, errMarshallerMustBeSpecified
	}
